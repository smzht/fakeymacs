...
| use_direct_input_in_vscode_terminal | パネルのターミナル内で４つのキー（C-k、C-r、C-s、C-y）のダイレクト入力機能を使うかどうかを            |
...
ダイレクト入力は常に機能します。こちらの利用形態の方が、ターミナルを利用しているかどうか
をプログラムに認識させる操作（パネルのターミナルを使う場合には必要）が不要となりますので、
シンプルな利用形態になると思います。
