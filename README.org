...
詳しくは、次の Fakeymacs key bindings manual の「■ 「Emacs 日本語入力モード」のキーバインド」
の項を参照してください。
- https'//github.com/smzht/fakeymacs/blob/master/fakeymacs_manuals/key_bindings.org
...
- 次の制約事項がありますので、ご理解の上ご利用ください。
 ...
 - リモートデスクトップ接続先でこの機能を利用する場合には、接続元、接続先に同じバージョンの Fakeymacs をインストールし、両側で fc.use_capslock_as_ctrl を True に設定し、リモートデスクトップ接続の「オプション」の「ローカルリソース」->「キーボード」で「このコンピューター」を選択してから接続してください。またこの選択をする場合、接続先での一部のキーバインドが変更となることにご留意ください。
