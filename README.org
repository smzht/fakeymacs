...
- CapsLock キーをマウスと組み合せて利用することできない制限事項があります
- 日本語キーボード設定となっている OS を利用している場合には、次の Issue に記載している制限事項があります。
 - https://github.com/smzht/fakeymacs/issues/42#issuecomment-2249829516
- VcXsrv などの X ウィンドウサーバソフトで、カーソル移動が遅くなる場合があります（WSLg は問題なし）
- CapsLock の状態やモディファイアキーの押下の状態がおかしくなった場合、一度 Keyhac の設定をリロードし、フックを OFF にして CapsLock の操作等により問題の状態を正常化させた後、再び フックを ON にする操作を行ってください
