...
を実行（管理者権限不要）し、Windows に再サインインすることで利用可能となります。

- [[/fakeymacs/blob/master/fakeymacs_tools/Subsititutes-00000411.reg]]
...
こちらは、かなりトリッキー方法で実現しており、いくつかの制約事項があります。
...
Windows 11 の場合、この設定は必須ように思われます。

- [[/fakeymacs/blob/master/fakeymacs_tools/LowLevelHooksTimeout.reg]]
