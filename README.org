...
[[/fakeymacs_manuals/keymap_layer.png]]
...
Fakeymacs は、Emacs のキーバインドの他、次の拡張機能で SpaceFn の機能もサポートしています。
Space キーを Fn として利用する際に発生する日本語入力時の変換漏れの問題や、Ctrl+Space
（set-mark-command）を入力する際に入力遅延が発生することにより生ずる問題を、Fakeymacs 独自の
方法で解決しており、Emacs のキーバインドとの共存が十分に可能なものとなっています。
一度お試しいただければと思います。
