...
# exec(readConfigExtension("config_extension-edit_mode.py"), dict(globals(), **locals()))

# # 「Emacs の場合、IME 切り替え用のキーを C-\ に置き換える」機能
# exec(readConfigExtension("config_extension-read_emacs.py"), dict(globals(), **locals()))

# # 「英語キーボード設定をした OS 上で、日本語キーボードを利用する場合の切り替えを行う」機能
# exec(readConfigExtension("config_extension-change_keyboard.py"), dict(globals(), **locals()))
