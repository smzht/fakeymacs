...
** Windows Terminal を利用する場合の留意事項について

Windows Terminal を利用する場合は、設定 -> 操作 メニューより、「貼り付けるときに末尾の空白を削除する」
をオフに設定してください。Microsoft Edit を使う際に複数行のコピペが正しく機能するようになります。

また、タブを切り替えた際にキーマップの再設定をしたい場合は、fc.update_keymap_key に設定した
キー（初期値：A-S-Enter）を入力してください。キーマップは C-Tab キーでタブを切り替えた際にも
再設定を行うようにしていますが、ターミナルの新規作成や削除により、開いているターミナルのキーマップ
に不整合が発生する場合があります。このような場合にこのキーをご利用ください。
...
Edit 終了後は、再度 A-S-Enter を入力することで、キーバインドを元にもどすことができます。
