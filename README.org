...
*** 10) 日本語キーボードの設定がされている OS で英語キーボードを使えるようにする設定を行う（オプション）

Fakeymacs は、日本語キーボードの設定がされている OS で英語キーボードを使えるようにする機能を
有しています。英語キーボードを接続していなくても、日本語キーボードを英語配列キーボードとして
使うこともできます。OS のキーボードレイアウトの設定が「日本語キーボード」になっていれば、
C-l で開くランチャーメニューの Other メニューにキーボードレイアウトを切り替えるメニューが表示
されます。fc.use_usjis_keyboard_conversion 変数を True にすれば、Keyhac 起動時にすぐ英語配列
キーボードとして使うこともできます。

また、この機能を使って日本語キーボードを英語配列キーボードとして使う場合、次の拡張機能を有効
にすることで、<］> キーを Enter キー、 <＼> キーを Shift キーとして利用することができるように
なります。（英語配列キーボードとして使わないキーを近くのキーに再割当てする機能です。）
