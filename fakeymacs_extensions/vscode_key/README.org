...
プレフィックスキーとして設定されているままのものがあること（Cursor ではプレフィックスキーが
Ctrl+m に置き換えられていますが、拡張機能の一部で Ctrl+k がプレフィックスキーとして設定されて
いるものがあります）やターミナルで kill-line のキーと重複するため、キーの変更が必要となります。
