...
Midnight Commander は Emacs like のキーバインドを持ち、ウィンドウタイトルも変更する
機能をもっているので、Windows Terminal などのターミナル上で Fakeymacs と共存して
利用可能です。EDITOR 環境変数の設定する方法などにより、利用するエディタを先に紹介
した Microsoft Edit にすると、とても使い勝手が良いです。お試しください。
