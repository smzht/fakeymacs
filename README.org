...
ウインドウ毎に使うキーバインドを決定しているため、Fakeymacs で Emacs キーバインドの割り当てを
行っているコマンドプロンプトや PowerShell と、Emacs キーバインドの割り当てを行わない Ubuntu
シェルなどが混在すると、適切なキーバインドの設定ができません。このため、適切なキーバインドの
再設定を行うためのキーを fc.update_keymap_key 変数に設定したキー（初期値：A-S-Enter）として
用意しました。また、このキーの機能は、C-Tab でタブを切り替える際にも自動で働くようにしています。
新しいターミナルを開いて適切なキーバインドが設定されていない場合には A-S-Enter を押し、その後は
C-Tab でターミナルを切り替えれば、それぞれのターミナルに適切なキーバインドが適用されます。
当然、ウィンドウの切り替えを行えば、その時点で開いているターミナルには適切なキーバインドが適用
されます（こちらは、従来からの機能です）。

このキーの新設により、Emacs キーバインドのオン／オフを切り替えるキーを C-S-Space から
A-S-Space に変更しました。Shift キーとの組み合わせは、Alt キーの方が使いやすいという判断からです。
キーは、fc.toggle_emacs_keybind_key 変数の設定により、変更することができます。
...
Edit をコマンドプロンプトや Windows PowerShall で起動した場合は、すぐに Emacs キーバインドが
使えるエディタとして機能します。Ubuntu シェルなど Linux で Edit を起動した場合は、一度 A-S-Enter
を入力することで、Emacs キーバインドで使えるようになります。
Edit 終了後は、再度 A-S-Enter を入力することで、もとのキーバインドに戻ります。

Linux 版の Edit は、次からダウンロードできます。一つのバイナリファイルとなっていますので、
これを edit というファイル名に置き換え、~/bin などのパスの通ったディレクトリに格納すると、
vim のようなエディタとして使うことができるようになります。
...
Microsoft Edit を使う際に複数行のコピペが正しく機能するようになります。
