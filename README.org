...
しないように対策していますので、fc.side_of_ctrl_key 変数で指定している側でない Ctrl キーを単押しするか、
マウスを使って操作するようにしてください。また、「貼り付けオプション」ボタンが不要な場合には、
次のページに記載に従い、ボタンを表示しない設定としてご利用ください。
...
● Microsoft Excel を使っていてセルの移動が遅いと感じる場合は、次のページの設定をお試しください。
私の環境ではかなりの改善がありました。専用のグラフィックボードを搭載していない PC では、お勧めの
設定と思われます。

- https://www.yonezu.net/blog/home/archives/000007.html
