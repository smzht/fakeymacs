...
このため、Fakeymacs ではウィンドウタイトルが変更されたイベントをキャッチすることにより、
タブ毎に異なるキーバインドの設定を実現しています。
...
なお、Windows Terminal で Microsoft Edit を利用する際は、設定 -> 操作 メニューより、
...
- https://hiroom2.jimdoweb.com/2015/09/06/midnight-commanderを使ってみる/#sec-3-4
...
マニュアル関連ページは以下を参照ください。

- https://wiki.archlinux.org/title/Midnight_Commander
- https://man.archlinux.org/man/mc.1

なお、Windows Terminal で Midnight Commander を利用する際は、設定 -> 二番目にある操作
メニューより、「全画面表示に切り替える」の Alt+Enter を削除してご利用ください。
「ファイル名取得」の機能が利用できるようになります。
Alt+Enter を削除しても、F11 が同機能のために利用可能です。

また、config_personal.py の [section-base-2] セクションにある次の設定を有効にすることを
検討ください。プレフィックスキーの Esc や C-x を使う際に IME を OFF にする機能が働き、
使い勝手が良くなると思います。
...
さらに、ファンクションキーの無い 60% キーボードなどをお使いの場合は、以降に紹介している
space_fn 拡張機能の利用を検討ください（space_fn 拡張機能の config_persona.py は、
config_personal_2.py を複写してご利用ください）。
Space+Ctrl+o でコマンドラインへの切り替え（Ctrl+o）が使えたり、Space+<数字キー>
でファンクションキーが使えるようになるなど、操作性が良くなると思います。

** WSL2 で Microsoft Edit と Midnight Commander を利用する場合の設定について

WSL2 で Microsoft Edit と Midnight Commander を利用する場合は、次の設定の利用を検討ください。
...
- ~/.config/mc/mc.ext （mc Version 4.8.30 より前）
#+BEGIN_EXAMPLE
regex/.*
    Open=wslstart %f
#+END_EXAMPLE

- ~/.config/mc/mc.ext （mc Version 4.8.30 以降）
#+BEGIN_EXAMPLE
[mc.ext.ini]
Version=4.0
...
wslstart は、次からダウンロードできます。wslstart の代わりに、sglstart の利用もお勧めです。
