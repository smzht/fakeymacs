...
※ vscode_prefix_key 変数は、VSCode のプレフィックスキーを使う際、後に入力した文字が IME の状態
によって全角で入力されることがある問題を対策するために設けた設定です。
...
**** ● VSCode 用プレフィックスキーの設定

|---------------------+------------------------+----------------------------|
| VSCode's Prefix Key | Fakeymacs's Prefix Key | Description                |
|---------------------+------------------------+----------------------------|
| C-k                 | C-A-k                  | vscode_prefix_key の初期値 |
|---------------------+------------------------+----------------------------|

※ Fakeymacs のプレフィックスキーを利用することにより、プレフィックスキーの後に入力するキー
によって全角文字が入力されることが無くなります。
