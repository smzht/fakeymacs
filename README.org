...
Fakeymacs は、Fresh Editor に対応しています。ただし、Fresh Editor は Linux版 Micro Editor
と同様、ターミナルのタイトルを置き換える機能を持っていません。
そこで、次のような alias を定義し、settitle コマンド経由で Fresh Editor が起動するようにして
ご利用ください。
...
Fakeymacs では、ターミナルのタイトルに「- fresh」が付いていることを検知すると Emacs キーバインド
にするようにしていますので、結果として Fresh Editor が Emacs キーバインドで利用できるようになります。
...
キーバインドで利用できるようになります。ノーマルモードとインサートモードのどちらでも Emacs
キーバインドが利用できるようになる他、ノーマルモードでは Vim のほとんどのキーバインドがそのまま
利用できますので、Vim と Emacs のハイブリッドのようなエディタとなります。
Vim のエキスパートの方には不要な拡張機能かもしれませんが、Emacsキーバインドユーザが環境条件に
より Vim を使う必要がでた場合に活用してください。
...
「Edit=edit %f」の箇所は、「Edit=settitle micro %f」または「Edit=settitle fresh %f」とすると、
先に紹介した Micro Editor や Fresh Editor が Emacs キーバインドで利用できるようになります。
...
Vim については、次の拡張機能により、キーバインドの設定を行っています。

|----------------+--------------------------|
| Extension name | Description              |
|----------------+--------------------------|
| [[/fakeymacs_extensions/vim_key][vim_key]]        | Vim 用のキーの設定を行う |
|----------------+--------------------------|
