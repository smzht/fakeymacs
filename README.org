...
本拡張機能の SpaceFn は、Space キーを Fn として利用する際に発生する日本語入力時の変換漏れの問題や
Ctrl+Space（set-mark-command）を入力する際の入力遅延によって発生する問題を Fakeymacs 独自の方法で
解決しており、Emacs のキーバインドとの共存が十分に可能なものとなっています。
一度お試しいただければと思います。
