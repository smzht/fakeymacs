...
※ Keyhac for Windows v1.82（旧版）を利用する場合、 Windows OS が x64版 でも
「Microsoft Visual C++ 再頒布可能パッケージ」は x86版 をインストールする必要があります。

※ Keyhac for Windows v1.83（最新版）を利用する場合、「Microsoft Visual C++ 再頒布可能
パッケージ」のインストールは不要と思われます。ただし、もし dll load エラーが発生し、
パッケージのインストールが必要と判断する場合には、x64 版をインストールする必要があると
思われます（未確認）。
