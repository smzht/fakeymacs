...
| esc_key_mode_in_keyboard_quit       | keyboard_quit 関数コール時の Esc キーの発行方法を指定する                                             |
|-------------------------------------+-------------------------------------------------------------------------------------------------------|
...
※ esc_key_mode_in_keyboard_quit の設定値には、次の３つの値のいずれかを指定できます。
- 1：Esc キーを常に発行する
- 2：Ctrl+g を２回連続押下した場合に Esc キーを発行する
設定値 2 の利用は、VSCode の Jupyter Extension を使う場合などを想定しています。
...
| Keybind | Function                 | VSCode Command      | Description                        |
|---------+--------------------------+---------------------+------------------------------------|
| C-A-g   | keyboard_quit3           | -                   | Esc キーを発行しない keyboard_quit |
