...
VSCode の Emacs Keymap Extension と比較した本機能の特徴は、以下のページの *<2021/02/23 追記>*
に記載しています。参考としてください。
...
● Keyhac のコンソールに「Time stamp inversion happened.」と表示される場合があります。これは、
レジストリ HKEY_CURRENT_USER\Control Panel\Desktop\LowLevelHooksTimeout を DWORD 形式で作成し、
そこに ms の値（10進数で 1000、5000 などの数値）を設定することで、ある程度の回避ができるようです。
