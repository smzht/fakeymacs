...
- 次の制約事項がありますので、ご理解の上ご利用ください。
 ...
 - リモートデスクトップ先でこの機能を利用する場合には、接続元、接続先に同じバージョンの Fakeymacs をインストールし、リモートデスクトップ接続のオプションのローカルリソース->キーボードで「このコンピューター」を選択してから接続してください。またこの選択をする場合、接続先での一部のキーバインドが変更となることにご留意ください。
