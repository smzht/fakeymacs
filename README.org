...
C-l で開くランチャーメニューの Other メニューで、キーボードレイアウトの切り替えが可能です。

この機能を利用して Keyhac 起動時すぐにキーボードを英語配列で使いたい場合には、
fc.use_usjis_keyboard_conversion 変数を True に設定してください。

また、日本語キーボードを英語配列キーボードとして使う場合には次の拡張機能を有効にすることで、
<］> キーを Enter キー、 <＼> キーを Shift キーとして利用することができるようになります。
（英語配列キーボードとして使わないキーを近くのキーに再割当てする機能です。）
