...
※ Keyhac for Windows v1.82（旧版）をインストールする場合は、 Windows OS が x64版 でも
「Microsoft Visual C++ 再頒布可能パッケージ」は x86版 をインストールする必要があります。

※ Keyhac for Windows v1.83（最新版）をインストールする場合は、「Microsoft Visual C++
再頒布可能パッケージ」のインストールは不要と思われますが、もしインストールが必要な場合は
x64 版をインストールする必要があると思われます（未確認）。
