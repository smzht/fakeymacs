...
を True としている）が行われていることにより、AI のインラインアシスタント機能用キーの
C-i が使えなくなることを回避するために行っています。この設定を行うことにより、VSCode
系エディタ側の C-i のキー設定が優先されるようになります。
...
| fc.vscode_setting<1> ～ <5>            | VSCode 用のキーバインドを利用するアプリケーションソフト、置き換えするプレフィックスキーの組み合わせ、置き換えするキーの組み合わせを指定する |
