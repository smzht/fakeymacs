...
Fakeymacs は、以降に記載している elecxzy_key 拡張機能を有効にすることで、elecxzy エディタの
Emacs キーバインドに次の機能を追加します。

- Meta キーとして Esc キーや C-[ キーを使えるようにする
- C-g キーの押下により、C-/ キーで undo と redo が切り替わるようにする
- kill_buffer コマンドに M-k キーを割り当てる

elecxzy エディタは、次のページからダウンロード可能です。

- https://github.com/kurouna/elecxzy

elecxzy エディタのキーバインドは、以下に記載があります。

- https://github.com/kurouna/elecxzy/blob/main/COMMANDLIST.md

なお、WSL2 の Linux から elecxzy エディタを使えるようにするには、~/bin 等のコマンドパスの
通ったフォルダに次の内容の elecxzy というコマンドを作成してお使いください。

#+BEGIN_EXAMPLE
#!/bin/bash

/mnt/c/Users/<user>/AppData/Local/Programs/elecxzy/elecxzy.exe "$(wslpath -wa "$1")" &
#+END_EXAMPLE

※ <user> の箇所は置き換える必要があります。
...
elecxzy エディタについては、次の拡張機能により、キーバインドの追加設定を行っています。

|----------------+------------------------------|
| Extension name | Description                  |
|----------------+------------------------------|
| [[/fakeymacs_extensions/elecxzy_key][elecxzy_key]]    | elecxzy 用のキーの設定を行う |
|----------------+------------------------------|
