...
fc.space_fn_window_keymap_list 変数に keymap_global を追加しても良いかもしれません。
...
（初期値は Space）から入力してください。本拡張機能の SpaceFn は Shift や Ctrl などの
その他のモディファイアキーと組み合わせて利用することもできますが、その他のモディファイア
キーを最初に入力した場合には、fc.space_fn_key 変数に指定したキーを入力した時点で、
キーの入力が確定します。（これは、Ctrl+Space や Shift+Space を遅延なく入力できるよう
する対策です。）
