...
このようなアプリの対応のため、Fakeymacs ではウィンドウタイトルが変更されたイベントをキャッチ
することにより、タブ毎に異なるキーバインドの設定を実現しています。
...
Fakeymacs は、Micro Editor に対応しています。ただし、Micro Editor は Microsoft Edit と異なり、
ターミナルのタイトルを置き換える機能を持っていません。そこで、ターミナルのタイトルを置き換える
コマンド settitle を作成しました。

- https://github.com/smzht/wsl-utils

この settitle コマンドをコマンドパスの通ったフォルダに置き、次のような alias を使って me を
起動すると、ターミナルのタイトルを置き換えた後に micro コマンドが起動するようになります。
...
次のページから Windows 版もダウンロードできますが、ターミナルのタイトルを置き換える仕組みは
独自に作成する必要があります。
...
space_fn 拡張機能を有効にすると、Space+Ctrl+o でコマンドラインへの切り替え（Ctrl+o）
が使えたり、Space+<数字キー> でファンクションキーが使えるようになるなど、操作性が
良くなると思います。

※ space_fn 拡張機能を利用する際は、config_persona.py に config_personal_2.py
を複写してご利用ください。
...
「Edit=edit %」の箇所は、「Edit=settitle micro」とすると、先に紹介した Micro Editor が
...
alias ph='eval $(history | tac | awk "{\$1=\"\"; print}" | peco --query "cd ")'
#+END_EXAMPLE

※ Ubuntu-24.04 の apt でインストールする peco は画面が崩れる場合があるようです。
その場合は、次の URL から最新のバージョンのものをダウンロードしてお使いください。
