...
Control キーとして利用する方法があります。
詳しくは、7) コンフィグレーションパラメータをカスタマイズする の項を参照してください。
...
※ 管理権限を持たない PC を利用している場合でレジストリによるキーの置き換えができない場合には、
fc.use_capslock_as_ctrl を True にすることで、CapsLock キーが Control キーとして利用できるようになります。
この機能を利用する場合、次の点に留意してください。
- fc.transparent_target に指定したアプリにはこの機能は適用されません
- CapsLock キーをマウスと組み合せて利用することができません
- VcXsrv などの X ウィンドウサーバソフトで、カーソル移動が遅くなる場合があります（WSLg は問題なし）
- CapsLock の状態やモディファイアキーの押下の状態がおかしくなった場合は、一度 Keyhac 設定をリロードし、フックを OFF にして CapsLock の操作等により問題の状態を正常化させた後、再び フックを ON にしてください
