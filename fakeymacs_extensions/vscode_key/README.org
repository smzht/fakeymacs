...
※ use_direct_input_in_vscode_terminal 変数が False（初期値）の場合には、 side_of_ctrl_key 変数で
設定した側でない Ctrlキーを使うか、C-q を前置することで４つのキーを利用することができます。
...
Terminal から抜ける場合は次のキーを利用するようにしてください。先に説明した４つのキーが VSCode の
