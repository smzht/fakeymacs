...
Cursor を利用する場合、Cursor には VSCode 用のキーマップ keymap_vscode が適用される他に Cursor
用のキーマップ keymap_cursor が適用されます。
このキーマップには、初期設定として次の設定が行われています。
...
|-------------------------------------+-------------------------------------+-------------|
| C-m                                 | C-A-m                               |             |
| fc.cursor_prefix_key[n][0] の設定値 | fc.cursor_prefix_key[n][1] の設定値 |             |
|-------------------------------------+-------------------------------------+-------------|
...
|---------+----------------------------+-------------|
| C-A-k   | self_insert_command("C-k") |             |
| C-A-l   | self_insert_command("C-l") |             |
|---------+----------------------------+-------------|

※ Cursor では、AI の機能を追加するために新たなショートカットキーが追加されています。これらの
ショートカットキーは Fakeymacs で設定している Emacs キーバインドと重複するものがあるため、
C-q を前置するか、fc.side_of_ctrl_key で設定した側でない Ctrl キーを使う方法などで入力する必要が
あります。本設定では、利用頻度の高い上記の２つのキーを置き換え設定しています。この置き換えは
次の設定により行われていますので、別の設定を追加したい場合には、以降の留意事項に書いている方法で
config_personal.py を作成し、同様の設定を追加してご利用いただければと思います。
