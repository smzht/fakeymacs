...
このため Fakeymacs では、ウィンドウタイトルが変更されたイベントをキャッチすることにより、
...
また、Midnight Commander は peco との相性が良いです。次のような使い方ができます。

#+BEGIN_EXAMPLE
cd() {
    builtin cd "$@" && history -s "cd $(printf "%q" "$(pwd)")"
}
alias ph='eval $(history | tac | awk "{\$1=\"\"; print}" | peco --query "cd ")'
#+END_EXAMPLE

Ubuntu-24.04 の apt でインストールする peco は画面が崩れる場合があるようですので、
そのような場合は次の URL から最新のバージョンのものをお使いください。
