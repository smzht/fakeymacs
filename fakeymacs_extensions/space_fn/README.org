...
（keymap_emacs キーマップでは、Esc をマルチストロークキー（Meta キー）として設定する場合があります。
この場合、複数のキーマップに渡るキーの置き換えが単純ではなくなるため、サンプル設定では除外しています。）
...
# 数引数のサポート
for n in range(10):
    define_key(keymap_emacs, f"U0-n", digit2(n))
#+END_EXAMPLE
