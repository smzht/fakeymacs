...
Fakeymacs は、以降に記載している elecxzy_key 拡張機能を有効にすることで、elecxzy の
Emacs キーバインドに次の機能拡張を行います。

- Meta キーとして Esc キーや C-[ キーを使えるようにする
- C-g キーの押下により、C-/ キーで undo と redo が切り替わるようにする
- kill_buffer コマンドに M-k キーを割り当てる

elecxzy は、次のページからダウンロード可能です。

- https://github.com/kurouna/elecxzy

elecxzy のキーバインドは、以下に記載があります。

- https://github.com/kurouna/elecxzy/blob/main/COMMANDLIST.md

なお、WSL2 の Linux から elecxy を使えるようにするには、~/bin 等のコマンドパスの通った
フォルダに次の内容の elecxy というファイル名のコマンドを作成してお使いください。
