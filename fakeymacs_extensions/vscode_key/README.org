...
なお、github.dev にアクセスするときなど（github のサイトで「.」を入力すると移行します）、
ブラウザのタブで VS Code Web を開きたい場合もあると思います。この場合に本拡張機能の設定を
利用したい場合には、ダブの切り替え後にキーバインドの再設定を行う必要があります。
その設定の仕方についても、このページの最後にある留意事項に記載していますので、
参照してください。
...
● ブラウザで VS Code Web 画面を開いた際に本拡張機能の vscode キーバインドを利用するためには、
次の３つの方法があります。

1. 以降に紹介している VS Code Web をアプリ化する方法などにより、専用のウィンドウで開くようにする
2. ブラウザで VS Code Web 画面を開いた後に、fc.update_keymap_key 変数で指定したキー（初期値：A-S-Enter）を入力する
3. トップフォルダの config_personal.py 内で、次のように fc.name_change_app_list 変数にブラウザアプリを登録し、タブの切替時にキーバインドの再設定が動作するようにする
...
※ （2025/06/16）上記の 3 を config.py での初期設定値とするように見直しました。
