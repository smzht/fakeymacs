...
● Mouse without Borders というアプリを利用している場合、Ctrl+Shift+S というショートカットキーの
設定がされています。これが、一般的な Windows アプリの「名前を付けて保存」の機能のショートカットキー
を上書きしてしまいます。Mouse without Borders アプリを利用している場合は、Mouse without Borders
アプリ側でこのショートカットキーを変更するか無効とするかなどの対策をお願いします。

- https://www.sriproot.net/blog/ctrl-shift-s-saveas-922

また、現在このアプリは PowerToys というアプリの「境界線のないマウス」という機能に統合されている
ようです。こちらの機能ではショートカットキーの見直しがされているようですので、PowerToys への
切り替えも検討ください。
