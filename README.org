...
という関数を発行しています。これは Command Palette で Terminal: Focus Terminal の起動を
しているのですが、"Te:Fo-Te" ではこのコマンドをユニークに特定できないため、コマンドリストで
一番上（一番最近起動した）に表示されるコマンドがマッチして起動されます。このため、一度
Command Palette で Terminal:Focus Terminal を起動し、コマンドリストの一番上でコマンドが
マッチするように調整してお使いください。（Terminal: Focus on Terminal View というコマンド
の利用も検討しましたが、Japanese Language Pack 利用時にコマンド名称が変わるので、採用を
見送りました。）
